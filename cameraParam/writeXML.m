...
camera_matrix = docNode.createElement('camera_matrix'); %创建mat节点
...
for i=1:3
    for j=1:3
        data.appendChild(docNode.createTextNode(sprintf('%e ',IntrinsicMatrix(i,j))));
    end
...
data = docNode.createElement('data');
for i=1:5
      data.appendChild(docNode.createTextNode(sprintf('%e ',Distortion(i))));
end
